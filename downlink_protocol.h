...
class Downlink : public ModuleBase<Downlink>, public px4::ScheduledWorkItem
{
public:
	Downlink();
	~Downlink() override;
	static Downlink *instantiate(int argc,char *argv[]);
...
private:
	...
	//uORB::Publication<actuator_servos_s> act_servo_pub{ORB_ID(actuator_servos)};
	uORB::Subscription airspeed_sub{ORB_ID(airspeed)};
	uORB::Subscription disco_data_sub{ORB_ID(disco_data)};
