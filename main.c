...
void wait_header(uint8_t byte,DWORD *i){
    if(buffer_index == FIRST_BYTE && byte == HI_BYTE){
        buffer[buffer_index++] = byte;
    }else if(buffer_index == SECOND_BYTE && byte == LOW_BYTE){
         buffer[buffer_index++] = byte;
...
void read_header(uint8_t byte,DWORD *i){
    buffer[buffer_index++] = byte;
    if(buffer_index == NUM_OF_HEADER){
        current_state = HEADER_CHECKSUM;
    } 
}

void header_checksum(uint8_t byte,DWORD *i){
    if(cal_header_checkd((const MESSAGE_HEADER_DOWNLINK*)buffer) == byte){
        buffer[buffer_index++] = byte;
        current_state = CHECK_LENGTH;
    }else{
        current_state = WAIT_IDEN;
        //printf("Error header checksum\n");
        buffer_index = 0;
    }
}

void check_length(uint8_t byte,DWORD *i){
    MESSAGE_HEADER_DOWNLINK* header = (MESSAGE_HEADER_DOWNLINK*)buffer;
    payload_len = header->length;
    if(payload_len > (MAX_PACKET_SIZE - SIZE_OF_HEADER)){
        current_state = WAIT_IDEN;
        //printf("Error size\n");
        buffer_index = 0;
    }else{
...
void read_payload(uint8_t byte,DWORD *i){
    buffer[buffer_index++] = byte;
    if(buffer_index >= (HEADER + payload_len + CRC_SIZE)){
        current_state = CHECK_CRC16;
    } 
}

void check_crc16(uint8_t byte,DWORD *i){
    uint16_t recv_crc = (buffer[buffer_index - 1] << 8) | buffer[buffer_index - 2];
    uint16_t cal_crc = GetCRC16(buffer, buffer_index - CRC_SIZE);
    if(cal_crc == recv_crc){
        (*i)--;
        current_state = COMMIT_DATA;
    }else{
        current_state = WAIT_IDEN;
        //printf("Error crc\n");
        buffer_index = 0;
...
void commit_data(uint8_t byte,DWORD *i){
    ...
    printf(" Checksum              : %u\n", last_data.checksum);
    // printf(" Protocol iden hi      : %02x\n", last_data.protocol_iden_hi);
    // printf(" Protocol iden low     : %02x\n", last_data.protocol_iden_low);
    // printf(" Protocol version      : %02x\n", last_data.protocol_version);
    // printf(" Take control          : %02x\n", last_data.take_control);
    // printf(" Source ID             : %02x\n", last_data.source_id);
    // printf(" Destination ID        : %02x\n", last_data.destination_id);
    // printf(" Sequence              : %02x\n", last_data.sequence);
    // printf(" Length                : %04x\n", last_data.length);
    // printf(" Unix time stamp       : %08x\n", last_data.unix_time_stamp);
    // printf(" Utc micro time stamp  : %08x\n", last_data.utc_microsecond_time_stamp);
...
int main() {
    ...
    while (1) {
        ...
        if(current_time - last_time >= TIME_INTERVAL){
            ...
            header.data.left_flap_cmd = -32767;
            header.data.right_flap_cmd = 32000;
            header.data.left_aileron_cmd = 100;
            header.data.right_aileron_cmd = 50;
            header.data.left_elevator_cmd = 1000;
            header.data.right_elevator_cmd = -2340;
            header.data.left_rudder_cmd = -3002;
            header.data.right_rudder_cmd = 3690;
            header.data.throttle_cmd = 32767;
            header.data.nose_wheel_cmd = -100;
            header.data.brake_cmd = 100;
        ...
        if (ReadFile(hSerial, rx_buffer, sizeof(MESSAGE_HEADER_DOWNLINK), &bytes_read, NULL) && bytes_read > 0) {
            void (*state_machine[])(uint8_t,DWORD*) = {wait_header,read_header,header_checksum,check_length,read_payload,check_crc16,commit_data};
            for(DWORD i = 0; i < bytes_read; i++){
                uint8_t n = rx_buffer[i];
                state_machine[current_state](n,&i);
            }
        }
    }
